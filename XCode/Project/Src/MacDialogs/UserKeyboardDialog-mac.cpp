...
#else
	// reverting old colour just means make it unselected on APPLE
	if (aColour == oldKeyColour)
...
#else
	// should open the select key dialog, but
	// pressing a key will assign it to the currently select key
	// on the virtual keyboard
	// That is: pressing a key will be like
...
// commands for main UK dialog
// press ok or cancel, close the dialog
// press any other key (on virtual keyboard) and it opens SelectKeyMapping dialogu
// with 'press a key' (optionally with SHIFTED too)
// When this is CLOSED it sets the user key mapping and
// opens the dialog again to select the SHIFTED version
